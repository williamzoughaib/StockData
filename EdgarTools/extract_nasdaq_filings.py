"""
SEC EDGAR 10-K/10-Q Extraction for Active NASDAQ-Traded Stocks
- Uses NASDAQ traded symbols only (no delisted companies)
- Extracts financials without XBRL using edgartools built-in parsing
- Supports incremental updates (only fetches new filings)
"""

import sys
sys.path.append('../HistoricalStockData')

from tradeable import get_nasdaq_symbols
...
def update_all_filings():
    ...
    """
    print("=" * 80)
    print("SEC EDGAR Filings Update - NASDAQ Traded Stocks Only")
    print("=" * 80)

    os.makedirs(OUTPUT_BASE_DIR, exist_ok=True)

    # Get NASDAQ traded symbols
    print("\nFetching NASDAQ traded symbols...")
    stocks, etfs = get_nasdaq_symbols()

    # Combine stocks and ETFs
    all_symbols = stocks + etfs
    print(f"\nProcessing {len(all_symbols)} active NASDAQ symbols...")
